...
const { sunglow, orangeRed, vividBurgundy, xandu, raisinBlack, cultured, google, facebook } =
  Colors;
...
export const SubTitle = styled.Text<PageTitleProps>`
  font-size: 30px;
  padding-left: 25px;
  align-self: flex-start;
  margin-bottom: 20px;
...
interface StyledButtonProps extends ViewProps, TouchableOpacityProps {
  google?: boolean;
  facebook?: boolean;
}
...
export const ButtonText = styled.Text<ButtonTextProps>`
  ...
  ${(props) =>
    (props.google === true || props.facebook === true) &&
    `
    padding: 0px 15px;
  `}
`;
...
interface ExtraViewProps extends ViewProps {
  signup?: boolean;
  signin?: boolean;
  diner?: boolean;
}

export const ExtraView = styled.View<ExtraViewProps>`
  flex-direction: row;
  justify-content: space-between;
  padding: 10px 0px;
  ${(props) =>
...
export const TextLinkContent = styled.Text<ExtraViewProps>`
  align-items: center;
  font-size: 15px;
  color: ${xandu};
  ${(props) =>
    (props.signup === true || props.signin === true || props.diner === true) &&
    `
    color: ${vividBurgundy};
    padding: 0px 10px;
  `}
